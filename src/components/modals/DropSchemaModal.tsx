...
export function DropSchemaModal() {
  const { dropSchemaModal, closeDropSchemaModal, tabs, closeTab } = useUIStore();
  const { isOpen, schema, tableCount } = dropSchemaModal;
  ...
  const isConfirmed = confirmText === schema;
  ...
  const handleDrop = async () => {
    if (!schema || !isConfirmed || !connectionId || !projectId) return;
    ...
    try {
      ...
      // Close all tabs related to this schema
      const schemaTabs = tabs.filter(
        (t) =>
          t.schema === schema &&
          (t.type === "table" || t.type === "edit-table" || t.type === "create-table")
      );
      schemaTabs.forEach((tab) => closeTab(tab.id));
      ...
      <div className="space-y-6">
        ...
        <div className="text-center">
          ...
          <p className="text-sm text-[var(--text-muted)] mt-2">
            ...
            <span className="font-mono text-red-400">{schema}</span> schema
            {tableCount !== null && tableCount > 0 && (
              <span>
                {" "}and all <span className="font-semibold text-red-400">{tableCount} {tableCount === 1 ? "table" : "tables"}</span> inside it
              </span>
            )}
        ...
        <div>
          <label className="block text-sm text-[var(--text-secondary)] mb-2">
            To confirm, type{" "}
            <span className="font-mono text-[var(--text-primary)]">{schema}</span>{" "}
            below
          </label>
          <input
            type="text"
